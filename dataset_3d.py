...
from utils import data_augmentation3d, normalize

def img_to_patches(img, win, stride=5):
    ...
    endw = img.shape[2]
    endh = img.shape[3]
    patch = img[:,0:endd-win+0+1:stride, 0:endw-win+0+1:stride, 0:endh-win+0+1:stride]
    total_pat_num = patch.shape[3]*patch.shape[1] * patch.shape[2]
    res = np.zeros([endc,win, win, win, total_pat_num], np.float32)
    maxI = img.max()  
    for ic in range(endc):
        for i in range(win):
            for j in range(win):
                for k in range(win):
                    sub_patch = img[ic, k:endd-win+k+1:stride, i:endw-win+i+1:stride, j:endh-win+j+1:stride]
                    res[ic,i, j, k, :] = np.array(sub_patch).reshape(total_pat_num)
    n_remove = 0
    #print(res.shape)
    for p_id in range(total_pat_num):
        p_id = p_id-n_remove
        if res[:,:,:,:,p_id].mean()<maxI*0.2:
            res = np.delete(res, p_id,4)
            n_remove += 1 
    return res
...
def prepare_data(data_path, \
    ...
    if gray_mode:
        traindbf = 'train_gray_d24.h5'
        valdbf = 'val_gray_d24.h5'
    else:
    ...
    with h5py.File(traindbf, 'w') as h5f:
        while i < len(files) and train_num < max_num_patches:
            ...
            tmp = nib.load(files[i])
            imgor = tmp.get_fdata()
            imgor = imgor/imgor.max()*255
            # h, w, c = img.shape
            #for sca in scales:
            ...
            print("\tfile: %s # samples: %d" % \
                  (files[i], patches.shape[4]*aug_times))
           ...
           #                   np.random.randint(0, 7))
                ...
                for mx in range(aug_times-1):
                    data_aug = data_augmentation3d(data, np.random.randint(1, 37))
...
class Dataset(udata.Dataset):
    ...
    def __init__(self, train=True, gray_mode=False, shuffle=False):
        ...
        if not self.gray_mode:
            ...
            self.traindbf = 'train_gray_d.h5'
            self.valdbf = 'val_gray_d.h5'
