...
def test_ffdnet(**args):
    ...
    if rgb_den:
        ...
        tmp = nib.load(args['input'])
        imorig = tmp.get_fdata()
        maxI = imorig.max()
        imorig = imorig/maxI*255
    ...
    # Add noise
    if args['add_noise']:
        noise1 = torch.FloatTensor(imorig.size()).\
                normal_(mean=0, std=args['noise_sigma'])
        noise2 = torch.FloatTensor(imorig.size()).\
                normal_(mean=0, std=args['noise_sigma'])
        imnoisy0 = imorig + noise1
        imnoisy = np.sqrt(imnoisy0**2+noise2**2)
    else:
    ...
    if not args['dont_save_results']:         
        ...
        x1 = nib.Nifti1Image(outimg*maxI/255, tmp.affine, tmp.header)
        nib.save(x1, 'Denoised.nii.gz')
        x2 = nib.Nifti1Image((noisyimg)*maxI/255, tmp.affine, tmp.header)
        nib.save(x2, 'NoisyImg.nii.gz')
