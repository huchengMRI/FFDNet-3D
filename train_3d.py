...
from models_3d_l5 import FFDNet
from dataset_3d import Dataset
from utils import weights_init_kaiming, batch_psnr, init_logger, \
            svd_orthogonalization_3d
...
def main(args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for i, data in enumerate(loader_train, 0):
            ...
            img_train = data
            noise_x = torch.zeros(img_train.size())
            noise_y = torch.zeros(img_train.size())
            stdn = np.random.uniform(args.noiseIntL[0], args.noiseIntL[1], \
                            size=noise_x.size()[0])
            for nx in range(noise_x.size()[0]):
                sizen = noise_x[0, :, :, :].size()
                noise_x[nx, :, :, :] = torch.FloatTensor(sizen).\
                                    normal_(mean=0, std=stdn[nx])
                noise_y[nx, :, :, :] = torch.FloatTensor(sizen).\
                                    normal_(mean=0, std=stdn[nx])
            img_train_nx = img_train + noise_x
            imgn_train = np.sqrt(img_train_nx**2 + noise_y**2)
            #imgn_train = img_train + noise_x
           ...
           # out_train = torch.clamp(imgn_train-model(imgn_train, stdn_var), 0., 1.)
            ...
            if training_params['step'] % args.save_every == 0:
                ...
                if not training_params['no_orthog']:
                    model.apply(svd_orthogonalization_3d)
        ...
        for valimg in dataset_val:
            ...
            size_val = img_val.size()
            noise_x = torch.FloatTensor(size_val).\
                                    normal_(mean=0, std=args.val_noiseL)
            noise_y = torch.FloatTensor(size_val).\
                                    normal_(mean=0, std=args.val_noiseL)
            img_val_nx = img_val + noise_x
            imgn_val = np.sqrt(img_val_nx**2 + noise_y**2)
            #imgn_val = img_val + noise_x
...
if __name__ == "__main__":
    ...
    parser.add_argument("--epochs", "--e", type=int, default=160, \
                     help="Number of total training epochs")
    parser.add_argument("--resume_training", "--r", action='store_true',\
                        help="resume training from a previous checkpoint")
    parser.add_argument("--milestone", nargs=2, type=int, default=[60, 80], \
    ...
    parser.add_argument("--noiseIntL", nargs=2, type=int, default=[0, 50], \
